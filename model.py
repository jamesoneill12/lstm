...
class LSTM(nn.Module):
    ...
    # only difference with forward_once is that we leave out htm1 and ctm1 initially
    def init_forward(self, x, lnum=0):
        z_in = torch.mm(x, self.hweight[lnum][:, :self.hid_dim]) + self.hbias[lnum][:, :self.hid_dim]
    ...
    def forward(self, x, h_t=None, all_out=True):
        ...
        for t in range(x.size(0)):
            for layer in range(self.hid_num):
                x_in = x[t, :, :]
                if t == 0:
                    h_t, c_t = self.init_forward(x_in)
                else:
                    h_t, c_t = self.forward_once(x_in, h_t, c_t)
            out.append(h_t)
        out = torch.cat(out, 1)
        # print(h_t)
        y = self.fc1(h_t)
